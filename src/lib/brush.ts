...
export function brushUp(
  ...
  select("#rect_" + cleanDimensionName).attr(
    "height",
    RECT_VALUE - heightTopRect - heightBottomRect,
  );
...
export function dragAndBrush(
  ...
  if (rectHeight < RECT_VALUE) {
    ...
    select("#triangle_up_hit" + cleanDimensionName).attr(
      "y",
      yPosRect + rectHeight,
    );
    select("#triangle_down_hit" + cleanDimensionName).attr("y", yPosTop);
...
export function filter(dimensionName: string, min: number, max: number): void {
  ...
  if (isDimensionCategorical(dimensionName)) {
    ...
    addRange(
      yScale.invert(rectBottom),
      parcoords.currentPosOfDims,
      dimensionName,
      "currentFilterBottom",
    );
    addRange(
      yScale.invert(rectTop),
      parcoords.currentPosOfDims,
      dimensionName,
      "currentFilterTop",
    );
...
export function filterWithCoords(
  topPosition: number,
  ...
  select("#rect_" + cleanDimensionName).attr("y", topPosition);
  select("#triangle_down_" + cleanDimensionName).attr("y", topPosition - 10);
  select("#triangle_up_" + cleanDimensionName).attr("y", bottomPosition);
  select("#rect_" + cleanDimensionName).attr("height", rectHeight);
...
function setToolTipDragAndBrush(
  ...
  if (
    (!invertStatus && tooltipValueTop == maxValue) ||
  ...
  if (
    (!invertStatus && tooltipValueBottom == minValue) ||
...
function checkAllPositionsBottom(
  positionItem: any,
  dimension: string,
  d: any,
  checkedLines: any[],
...
export function addSettingsForBrushing(
  dimension: string,
  ...
  const dimensionSettings = parcoords.currentPosOfDims.find(
...
export function addInvertStatus(
  status: any,
  dimensionName: any,
  key: any,
): void {

...
class SteerableParcoords {
    removeDuplicateColumnNames(value) {
        ...
        let column_string = csvParse(complete_arr[0]);
        let n = 0;
    ...
    invert(dimension, newFeatures, xScales, yScales, yAxis, active, inactive) {
        ...
        const textElement = select$1(invert_id);
        ...
        select$1(dimension_id).call(yAxis[dimension].scale(yScales[dimension].domain(yScales[dimension]
            .domain().reverse())))
            .transition();
        // force update lines
        active.attr('d', (d) => {
    ...
    setupYScales(height, padding, features, newDataset) {
        ...
        features.map(x => {
            ...
            if (isNaN(testValue[0]) !== false) {
                ...
                Math.min(...newDataset.map(o => o[x.name]));
    ...
    setupYAxis(features, yScales, newDataset) {
        let counter = 0;
        const limit = 30;
        let yAxis = {};
        Object.entries(yScales).map(key => {
            ...
            if (isNaN(temp_var_values[0])) {
                ...
                if (unique_arr.length > limit) {
                    ...
                    yAxis[key[0]] = axisLeft(key[1]).tickValues(filtered_arr);
                }
                else {
                    yAxis[key[0]] = axisLeft(key[1]).tickValues(temp_var_values);
                }
            }
            else {
                yAxis[key[0]] = axisLeft(key[1]);
            }
        });
        return yAxis;
    }
    ...
    generateSVG(content, newFeatures) {
        ...
        let tooltip_path = select$1('#parallelcoords')
            .append('g')
            .style("position", "absolute")
            .style("visibility", "hidden");
        const svg = select$1("#parallelcoords")
            .append('svg')
        ...
        var tooltip_dim = select$1('#parallelcoords')
            .append('g')
            .style("position", "absolute")
            .style("visibility", "hidden");
        featureAxisG
        ...
        window.onclick = (event) => {
            if (!(event.ctrlKey || event.metaKey)) {
                for (let i = 0; i < ids.length; i++) {
                    select$1('.' + ids[i]).style("stroke", "rgb(0, 129, 175)");
                }
            }
        };
    }
    ...
    highlight(data) {
        ...
        if (data.length !== 0) {
            ...
            for (let i = 0; i < temp_text.length; i++) {
                let isOrange = select$1("." + temp_text[i].replace(/,./g, "")).style("stroke");
    ...
    doNotHighlight(event, i, selected_path) {
        if (selected_path !== "") {
            ...
            let new_temp_text = [];
            for (let i = 0; i < temp_text.length; i++) {
                let isOrange = select$1("." + temp_text[i]).style("stroke");
                if (isOrange !== "rgb(255, 165, 0)") {
...
const { loadCSV, invert, setDimensions, generateSVG, removeDuplicateColumnNames, checkIfDuplicatesExists, select, position, onDragStartEventHandler, onDragEventHandler, transition, onDragEndEventHandler, onInvert, prepareData, setupYScales, setupXScales, setupYAxis, setupBrush, onBrushEventHandler, applyFilters, selected, initContent, linePath, highlight, doNotHighlight, createTooltipForPathLine, getAllPointerEventsData } = new SteerableParcoords();

export { applyFilters, checkIfDuplicatesExists, createTooltipForPathLine, doNotHighlight, generateSVG, getAllPointerEventsData, highlight, initContent, invert, linePath, loadCSV, onBrushEventHandler, onDragEndEventHandler, onDragEventHandler, onDragStartEventHandler, onInvert, position, prepareData, removeDuplicateColumnNames, select, selected, setDimensions, setupBrush, setupXScales, setupYAxis, setupYScales, transition };
